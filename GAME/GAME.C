...
static void chk_exit(void) {
  static char msg[]={'è'^85,'Æ'^85,'¶'^85,'†'^85,'´'^85,'„'^85,'©'^85,'·'^85,
    '‚'^85,'†'^85,' '^85,
    'Ø'^85,'•'^85,'‡'^85,'•'^85,'„'^85,'·'^85,'‚'^85,'†'^85,'≠'^85,'Æ'^85,
...
void G_act(void) {
  ...
  if(g_exit==1) {
	...
	if(g_st==GS_INTER) {
	  F_loadmus("INTERMUS");
	}else {F_loadmus("äéçÖñ");if(mus_vol>0) mus_vol=128;}
	S_startmusic();
  }else if(g_exit==2) {
	switch(g_map) {
...
static void pl_info(player_t *p,int y) {
  ...
  Z_gotoxy(25,y);Z_printbf("ìÅàã");
  Z_gotoxy(25,y+15);Z_printbf("ìÅàâëíÇ Ç åàçìíì");
  Z_gotoxy(25,y+30);Z_printbf("çÄòÖã ëÖäêÖíéÇ %u àá %u",p->secrets,sw_secrets);
...
void G_draw(void) {
  ...
  switch(g_st) {
    ...
    case GS_INTER:
	  V_pic(0,0,scrnh[1]);
	  Z_gotoxy(60,20);Z_printbf("ìêéÇÖçú èêéâÑÖç");
	  Z_calc_time(g_time,&hr,&mn,&sc);
	  Z_gotoxy(115,40);Z_printbf("áÄ %u:%02u:%02u",hr,mn,sc);
	  h=60;
	  if(_2pl) {
		Z_gotoxy(80,h);Z_printbf("èÖêÇõâ àÉêéä");
		Z_gotoxy(80,h+70);Z_printbf("Çíéêéâ àÉêéä");
		h+=20;
	  }
	  pl_info(&pl1,h);
