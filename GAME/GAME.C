...
static void chk_exit(void) {
  static char msg[]={''^85,'®'^85,'¦'^85,' '^85,'«'^85,'γ'^85,'©'^85,'α'^85,
    'β'^85,' '^85,' '^85,
    '―'^85,'¥'^85,'ΰ'^85,'¥'^85,'γ'^85,'α'^85,'β'^85,' '^85,'­'^85,'®'^85,
    'Ά'^85,'¨'^85,'β'^85,'¥'^85,' '^85,'¨'^85,'£'^85,'ΰ'^85,'γ'^85,0};
  int i;
...
inter:
	...
	if(g_st==GS_INTER) {
	  F_loadmus("INTERMUS");
	}else {F_loadmus("…–");if(mus_vol>0) mus_vol=128;}
	S_startmusic();
  }else if(g_exit==2) {
	switch(g_map) {
...
static void pl_info(player_t *p,int y) {
  ...
  Z_gotoxy(25,y);Z_printbf("“‹");
  Z_gotoxy(25,y+15);Z_printbf("“‰‘’‚ ‚ “’“");
  Z_gotoxy(25,y+30);Z_printbf("€…‹ ‘……’‚ %u ‡ %u",p->secrets,sw_secrets);
...
void G_draw(void) {
  ...
  switch(g_st) {
    ...
    case GS_INTER:
	  V_pic(0,0,scrnh[1]);
	  Z_gotoxy(60,20);Z_printbf("“‚… ‰„…");
	  Z_calc_time(g_time,&hr,&mn,&sc);
	  Z_gotoxy(115,40);Z_printbf("‡€ %u:%02u:%02u",hr,mn,sc);
	  h=60;
	  if(_2pl) {
		Z_gotoxy(80,h);Z_printbf("…‚›‰ ƒ");
		Z_gotoxy(80,h+70);Z_printbf("‚’‰ ƒ");
		h+=20;
	  }
	  pl_info(&pl1,h);
