...
static char *main_txt[]={
  ...
  "НАЧАТЬ ЗАНОВО","ГРОМКОСТЬ","ЯРКОСТЬ"
},*ngplr_txt[]={
...
static menu_t main_mnu={
  ...
  MENU,3,0,75,"РАЗНОЕ",opt_txt,opt_typ
},ngplr_mnu={
  ...
  MSG,0,0,0,"ВЫ ДУМАЕТЕ, ТАК ПРОСТО ВЫЙТИ ИЗ DOOM'А 2D?",NULL,quit_typ
},quit2_msg={
  MSG,0,0,0,"ХОРОШЕНЬКО ПОDOOMАЙТЕ ПЕРЕД ТЕМ КАК ВЫЙТИ",NULL,quit_typ
},quit3_msg={
  MSG,0,0,0,"DOOM2D ЛУЧЩЕ WINDOWS'95 И OS/2 ВМЕСТЕ ВЗЯТЫХ?",NULL,quit_typ
},endgm_msg={
...
int GM_act(void) {
  ...
  if(input) switch(lastkey) {
    ...
    case 0x0E: if(icur) {ibuf[--icur]=0;GM_set(mnu);} break;
    default:
      if(icur>=23) break;
