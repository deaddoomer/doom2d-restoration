...
void F_set_snddrv(void) {
  ...
  logo("F_set_snddrv: card #%d\n",snd_card);
  snd_drv=snd_drv_tab[snd_card];
  logo("  %s ",S_getinfo());
  if(snd_card) logo("(%dHz)\n",(dword)sfreq);
    else logo("\n");
}
...
int F_getresid(char *n) {
  ...
  ERR_fatal("F_getresid: %.8s not found",n);
  return -1;
}
...
void F_loadmap(char n[8]) {
  ...
  if(memcmp(hdr.id,"Doom2D\x1A",8)!=0)
	ERR_fatal("%.8s is not a Doom2D map!",n);
  for(;;) {
	...
	if(!IT_load(h))
	if(!SW_load(h))
	  ERR_fatal("Unknown block %d(%d) in %.8s",blk.t,blk.st,n);
	lseek(h,o,SEEK_SET);
  }
}
...
void F_loadmus(char n[8]) {
  ...
  for(i=0;i<inum;++i) {
	...
	if(!(dmi[i]=malloc(wad[r].l))) continue;
	F_loadres(r,dmi[i],0,wad[r].l);
	lseek(h,o,SEEK_SET);
  }
